...
struct DescartesGui {
    ...
    /// Command channel for ScudBridge
...
impl DescartesGui {
        ...
        match message {
            ...
            Message::ScudEvent(event) => {
                ...
                match event {
                    ...
                    ScudEvent::WavesComputed(wave_ids) => {
                        // Convert wave IDs to full TaskInfo by matching loaded tasks
    ...
    fn view_waves(&self) -> Element<'_, Message> {
        ...
        if self.state.waves.is_empty() {
            ...
            for (wave_idx, wave) in self.state.waves.iter().enumerate() {
                ...
                for task in wave {
                    ...
                    if task.status != "Done" && task.status != "done" {
                        task_actions = task_actions
                            .push(button("Done").on_press(Message::MarkTaskComplete { task_id: task.id.clone() }));
                    }
                    if task.status != "Blocked" && task.status != "blocked" {
                        task_actions = task_actions
                            .push(button("Block").on_press(Message::MarkTaskBlocked { task_id: task.id.clone() }));
                    }
        ...
        let tag_label = if let Some(ref tag) = self.state.active_tag {
            text(format!("Tag: {}", tag))
        } else {
            text("All tasks")
        };
    ...
    fn view_agents(&self) -> Element<'_, Message> {
        ...
        match self.state.agent_status {
            AgentStatus::Idle => {
                // Swarm start controls when idle - use active tag or default
        ...
        // Show active tag
        let tag_info = if let Some(ref tag) = self.state.active_tag {
            text(format!("Active tag: {}", tag))
        } else {
            text("No tag selected (using default: refactor)")
        };
    ...
    fn view_output(&self) -> Element<'_, Message> {
        // Header with current task and controls
        let task_status = if let Some(ref task_id) = self.state.current_task {
            text(format!("Task: {} | Status: {:?}", task_id, self.state.agent_status))
        } else {
            text(format!("Status: {:?}", self.state.agent_status))
        };
...
mod tests {
    ...
    /// Test ScudEvent::WavesComputed uses loaded tasks to build waves
    #[test]
    fn test_scud_event_waves_computed() {
        ...
        let _ = app.update(Message::ScudEvent(ScudEvent::TasksLoaded(tasks)));

        // Then receive computed waves (as IDs)
        let wave_ids = vec![
            vec!["1".to_string(), "2".to_string()],
            vec!["3".to_string()],
        ];
        let _ = app.update(Message::ScudEvent(ScudEvent::WavesComputed(wave_ids)));

        // Waves should be populated with TaskInfo from tasks
        assert_eq!(app.state.waves.len(), 2);
        assert_eq!(app.state.waves[0].len(), 2);
